    ...
    <div wire:key="{{ $todo->id }}"
        ...
        <div class="flex justify-between space-x-2">
            ...
            <div class="flex items-center">
                @if ($todo->completed)
                    <input wire:click="toggel({{ $todo->id }})" type="checkbox" class="mr-2" checked>
                @else
                    <input wire:click="toggel({{ $todo->id }})" type="checkbox" class="mr-2 ">
                @endif

                @if ($editingTodoID == $todo->id)
                    <div class="">
                        <input wire:model="editingTodoName" type="text" placeholder="Todo.." name="editingTodoName"
